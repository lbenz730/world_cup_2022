...
make_table <- function(Group = 'all', by_group = T, alive_round = NULL) {
  if(Group == 'all') {
    df <- df_stats
    subtitle <- ''
    if(by_group) {
  ...
  if(!is.null(alive_round)) {
   df <- df[ df[[alive_round]] > 0, ]
  }
...
ko_table <-
  ...
  fmt_percent(columns = any_of(c('qf', 'sf', 'finals', 'champ')), decimals = 0, sep_mark = '') %>%

  ### Align Columns
  cols_align(align = "center") %>%

  data_color(columns = any_of(c('qf', 'sf', 'finals', 'champ')),
             colors = scales::col_numeric(palette = ggsci::rgb_material('amber', n = 100), domain = c(0, 1))) %>%
  data_color(columns = c(alpha),
             colors = scales::col_numeric(palette = ggsci::rgb_material('amber', n = 100), domain = range(df_stats$alpha))) %>%
  data_color(columns = c(net_rating),
  ...
  tab_spanner(label = 'Ratings', columns = c('alpha', 'delta', 'net_rating')) %>%
  tab_spanner(label = 'Knockout Round', columns = any_of(c('qf', 'sf', 'finals', 'champ'))) %>%
